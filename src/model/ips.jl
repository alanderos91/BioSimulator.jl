...
execute_jump!(state, model::Vector{IPSReactionStruct}, j) = execute_jump!(state, model[j])
...
##### convenience wrapper
rate(reaction::Vector{IPSReactionStruct}, lattice, j) = rate(reaction[j], lattice)
