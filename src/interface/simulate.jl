...
function simulate{T}(model::Network, algorithm::Type{T}=SSA;
  ...
  sim_data = pmap((i) -> begin
    alg     = algorithm(time; kwargs...)
    output  = SimData(eltype(x0), c, epochs)
    simulate!(output, copy(x0), alg, deepcopy(rxn), t_index)
  end, 1:trials)

  S = typeof(sim_data[1])

  return convert(Array{S}, sim_data)
end
...
function simulate!(
  ...
  while !done(algorithm)
    ...
    j = update!(output, Xt, t, t_index, j)
