...
- `output`: The type of output to record. The `:full` option will record simulation history after every algorithm step, whereas `:partial` will record the history after according to `sampling_interval`.
- `sampling_interval`: The time between updates when `output=:partial`. The default is to record after every unit of time, according to the scale implicit in a given model `m`.
...
"""
function simulate(network::Network; method::Symbol=:SSA, time=1.0, output=:partial, sampling_interval=1.0, realizations=1, track=Symbol[], kwargs...)
  ...
  otype = (output == :full) ? Explicit() : Uniform()
  ...
  Xt_history = initialize_history(otype, Xt, id2ind, realizations, Int(time / sampling_interval) + 1, time)
...
function initialize_history(::Explicit, Xt, id2ind, nrlz, nstates, t)
  return SimData(Xt, nrlz, id2ind)
end

function initialize_history(::Uniform, Xt, id2ind, nrlz, nstates, t)
  return SimData(Xt, nrlz, id2ind, nstates, t)
end
