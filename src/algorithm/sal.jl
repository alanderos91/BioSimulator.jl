type SAL <: Algorithm
    ...
    function SAL(args)
        ...
        new(tol, thrsh, ctrct, 0.0, Float64[], Float64[], Int[], 0, 0, 0.0, 0.0, 0, [:avg_ssa, :avg_sal])
    end
end
...
function compute_statistics(alg::SAL)
    alg.avg_ssa = cumavg(alg.avg_ssa, alg.ssa_steps, alg.counter)
    alg.avg_sal = cumavg(alg.avg_sal, alg.sal_steps, alg.counter)
